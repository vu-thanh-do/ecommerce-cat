...
const MyCart = () => {
  ...
  const filterItems = items?.filter((itc: any) => (checkSelect ? itc.isFavorite == true : !itc.isFavorite))
  /* Tính tổng tiền và tổng số lượng quantity */
  const { total, quantity } = filterItems.reduce(
    (accumulator, item) => {
    ...
    <div className='sidebar shrink-0 w-[300px] bg-[#fff] text-[14px] rounded-sm mx-[16px] pb-[12px] h-fit hidden lg:block'>
      <div className='border border-transparent border-b-[#f1f1f1]  px-4 py-2 '>
        <div className='flex justify-between items-center'>
          <div className='uppercase font-semibold'> {!checkSelect ? 'Giỏ hàng của tôi' : 'Danh mục yêu thích'}</div>
          <div
        ...
        <Button onClick={() => setCheckSelect(!checkSelect)}> {!checkSelect ? 'Xem yêu thích' : 'Xem Giỏ hàng'}</Button>
      </div>
      ...
      <div className='mx-[16px]'>
        <div className='max-h-[450px] overflow-y-scroll hidden-scroll-bar'>
          {items.length > 0 &&
            items
              .filter((itc: any) => (checkSelect ? itc.isFavorite == true : !itc.isFavorite))
              .map((item) => <CardOrder key={uuidv4()} product={item} />)}
        </div>
        <div className='cart '>
