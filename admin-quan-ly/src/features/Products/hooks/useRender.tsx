          ...
          <div className='flex flex-col gap-1'>
            ...
            {toppings.slice(0, 2).map((topping: IToppingRefProduct) => (
              <div key={topping._id} className='relative grid grid-cols-2'>
                <p className='border-r-graydark w-full pr-3 uppercase '>{topping.name}</p>
              </div>
            ))}
          </div>
  ...
  const handleDeleteProduct = async (id: string) => {
    try {
      ...
      if (response.message === 'success') {
        message.success(' đã được chuyển vào thùng rác!')
      }
    } catch (error) {
          ...
          <>
            <Space>
              ...
              {user.role === IRoleUser.ADMIN ? (
                <Popconfirm
                  ...
                  description={` sẽ được ${product.is_active ? 'ẩn đi!' : 'hiển thị'}`}
            ...
            <Space>
              <Popconfirm
                ...
                cancelText='Hủy'
              >
                <ButtonAntd size='large' icon={<DeleteIcon />} danger className=' flex items-center justify-center ' />
              </Popconfirm>
            </Space>
          </>
        )
