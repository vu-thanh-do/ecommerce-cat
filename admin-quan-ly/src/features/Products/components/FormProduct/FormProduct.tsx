...
const FormProduct = () => {
  ...
  const handleSubmitForm = async (values: any) => {
    ...
    //   message.error('Phải có ít nhất 1 loại ')
    //   return
    // }
  ...
  useEffect(() => {
    if (productEdit) {
      form.setFieldsValue({
        ...
        sex: productEdit.sex ? "Giống cái" : "Giống đực",
        origin: productEdit.origin ?"Nhập khẩu" :"Trong nước",
  ...
  return (
    <Drawer
      title={`${productId === null ? 'Thêm' : 'Cập nhật'} mặt hàng`}
      placement='right'
      width={850}
      destroyOnClose
      onClose={() => {
      ...
      <Form layout='vertical' autoComplete='off' form={form} onFinish={handleSubmitForm}>
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item
              ...
              label='Tên mèo'
              rules={[
                { required: true, message: 'Tên mèo là bắt buộc!' },
                {
                  validator: (_, value) => {
                    if (value.trim() === '') {
                      return Promise.reject('Tên mèo không được chứa toàn khoảng trắng!')
                    }
            ...
            >
              <Input placeholder='Tên mèo' size='large' />
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item name='category' label='Giống mèo' rules={[{ required: true, message: 'Giống mèo là bắt buộc' }]}>
              <Select placeholder='Giống mèo' size='large'>
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item
              name='toppings'
              label='Lựa chọn Đặc điểm'
              rules={[{ required: true, message: 'Đặc điểm là bắt buộc' }]}
            >
              <Select size='large' mode='multiple' allowClear placeholder='Lựa chọn Đặc điểm'>
          ...
          <Col span={12}>
            <Form.Item name='timBooking' label='Tính cách'>
              <Input placeholder='Tính cách' className='w-full' />
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item name='sex' label='Giới tính' rules={[{ required: true, message: 'Giới tính là bắt buộc' }]}>
              <Select placeholder='Chọn Giới tính' size='large'>
                <Option value={false}>Giống đực</Option>
                <Option value={true}>Giống cái</Option>
              </Select>
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item name='origin' label='Nguồn gốc' rules={[{ required: true, message: 'Nguồn gốc là bắt buộc' }]}>
              <Select placeholder='Chọn Nguồn gốc' size='large'>
                <Option value={false}>Trong nước</Option>
                <Option value={true}>Nhập khẩu</Option>
              </Select>
            </Form.Item>
          </Col>
        </Row>
        {/* <Row gutter={16}>
          <Col span={12}>
            <Form.Item className='w-full' label='Size '>
              <Form.List name='size'>
                {(fields, { add, remove }) => (
                  <>
                    {fields.map(({ key, name, ...restField }) => (
                      <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align='baseline'>
                        ...
                        >
                          <InputNumber size='large' placeholder='Giá size của ' className='w-full' />
                        </Form.Item>
          ...
          <Col span={12}>
            <Form.Item className='w-full' label='Loại '>
                  ...
                  <>
                    {fields.map(({ key, name, ...restField }) => (
                      <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align='baseline'>
                        ...
                        >
                          <InputNumber size='large' placeholder='Giá  của ' className='w-full' />
                        </Form.Item>
                        <MinusCircleOutlined onClick={() => remove(name)} />
                      </Space>
                    ))}
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item name='weight' label='Cân nặng'>
              <InputNumber placeholder='Cân nặng' className='w-full' />
            </Form.Item>
          </Col>
        </Row>
        {/* <Row gutter={16}>
          ...
          <Col span={24}>
            {!isUpload && !isLoading && (
              <Form.Item
                ...
                className='w-full'
                label='Hình ảnh '
                rules={[{ required: productEdit ? false : true, message: 'Không được để trống hình ảnh ' }]}
              >
