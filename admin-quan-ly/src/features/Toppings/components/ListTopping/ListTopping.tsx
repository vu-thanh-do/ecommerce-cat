...
const ToppingList = () => {
  ...
  const toppings = toppingsList
    ?.filter((itc: any) => itc.owner == user._id)
    .map((topping, index) => ({ ...topping, key: topping._id, index: index }))
    ...
    <div>
      <Space>
        {user && user.role === IRoleUser.ADMIN && hasSelected && (
          <Popconfirm
            title='Bạn thực sự muốn xóa những Đặc điểm này?'
            description='Hành động này sẽ xóa những Đặc điểm đang được chọn!'
            onConfirm={start}
            className='ml-[10px]'
          >
            <ButtonAntd
